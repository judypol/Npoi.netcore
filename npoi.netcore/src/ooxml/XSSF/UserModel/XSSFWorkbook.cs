...
namespace NPOI.XSSF.UserModel
{
    ...
    public class XSSFWorkbook : POIXMLDocument, IWorkbook
    {
        ...
        public ISheet GetSheet(String name)
        {
            ...
            {
                if (name.Equals(sheet.SheetName, StringComparison.CurrentCultureIgnoreCase))
                {
                    return sheet;
                }
            }
        ...
        public void RemoveName(String name)
        {
            for (int i = 0; i < namedRanges.Count; i++)
            {
                ...
                if (nm.NameName.Equals(name,StringComparison.CurrentCultureIgnoreCase))
                {
                    RemoveName(i);
                    return;
                }
        ...
        */
        ...
        public XSSFName GetBuiltInName(String builtInCode, int sheetNumber)
        {
            foreach (XSSFName name in namedRanges)
            {
                if (name.NameName.Equals(builtInCode, StringComparison.CurrentCultureIgnoreCase)
        ...
        private bool ContainsSheet(String name, int excludeSheetIdx)
        {
            ...
            for (int i = 0; i < ctSheetArray.Count; i++)
            {
                ...
                if (excludeSheetIdx != i && name.Equals(ctName, StringComparison.CurrentCultureIgnoreCase))
                    return true;
            }
            return false;
        }
